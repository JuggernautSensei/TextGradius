...
콘솔 창에서 실행되는 텍스트 기반 슈팅 게임입니다. C++20으로 작성했으며 Windows(MSVC) 환경에서 동작합니다. 고전 슈팅 게임 Gradius에서 영감을 받아, 우주선을 조종해 적을 물리치고 점수를 획득하는 게임을 처음부터 직접 만들었습니다.

![게임플레이](Resource/GamePlay.png)

<video src="Resource/DemoPlay.mp4" controls width="720"></video>
...
클래스, 구조체, 포인터, 동적 메모리 할당 등 C++의 기본 문법을 실습하고 OOP를 실제 설계에 적용하면서, 가독성과 유지보수성을 고려한 구조를 고민할 수 있었습니다. STL도 적극 활용해 코드 효율을 높이는 방법을 함께 고민했습니다.
...
![PrintAt / Present](Resource/ConsoleRender.png)
...
게임은 다양한 GameObject로 구성되어 있으며, 각 GameObject는 게임 내에서 특정 역할을 수행합니다. 플레이어 캐릭터, 적 캐릭터, 총알, 아이템 등이 있으며, 각 GameObject는 독립적으로 동작하고 필요한 경우 다른 GameObject와 상호작용합니다. 이를 위해 `GameObject`라는 추상 클래스를 정의하고, 각 오브젝트는 이 클래스를 상속받아 구현했습니다.
...
![랭킹](Resource/Ranking.png)

<video src="Resource/RankingDemo.mp4" controls width="720"></video>
...
이 프로젝트를 통해 가장 크게 향상시킬 수 있었던 역량은 디버깅 능력과 성능 병목을 찾아내는 능력이었습니다. 문제를 조용히 감추는 설계보다는, 프로그램 곳곳에 ASSERT를 심어두고 문제가 발생하면 즉시 크래시를 일으켜 그 자리에서 디버깅으로 원인을 해결하는 방식으로 개발을 진행했습니다.
